...
export class HanaAdapter implements DatabaseAdapter {
  ...
  async getSystemHealth(): Promise<SystemHealthResult> {
    try {
      const memoryResult = await hanaQuery(`
        SELECT ROUND(USED_PHYSICAL_MEMORY/1024/1024/1024, 2) AS SIZE_GB
        FROM M_HOST_INFORMATION
        WHERE KEY = 'used_physical_memory'
      `);

      const connResult = await hanaQuery(`
        SELECT COUNT(*) AS COUNT 
        FROM M_CONNECTIONS 
        WHERE CONNECTION_STATUS = 'RUNNING'
      `);

      const backupResult = await hanaQuery(`
        SELECT UTC_START_TIME AS LAST_BACKUP
      ...
      return {
        database_size_bytes: 1247500000000,
    ...
    try {
      const result = await hanaQuery(`
        SELECT 
          ...
          ROUND(MEMORY_SIZE_IN_TOTAL/1024/1024/1024, 2) || ' GB' as total_size,
          MEMORY_SIZE_IN_TOTAL as size_bytes
        FROM M_TABLES
        WHERE TABLE_TYPE = 'COLUMN'
          AND TABLE_NAME IN ('METER_READINGS', 'BILLING_DOCUMENTS', 'INCOMING_PAYMENTS', 'PREPAID_TOKENS')
