...
async function seed() {
  ...
  for (let i = 0; i < customerIds.length; i++) {
    ...
    // Equipment (meter) — capture serial number for prepaid tokens FK
    const serialNumber = `SEC-${randInt(10000000, 99999999)}`;
    ...
    // Prepaid tokens (if prepaid) — use the actual meter serial number
    if (isPrepaid) {
